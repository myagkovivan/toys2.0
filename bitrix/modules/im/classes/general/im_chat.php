...
class CIMChat
{
	...
	function GetLastMessage($toChatId, $fromUserId = false, $loadExtraData = false, $bTimeZone = true, $limit = true)
	{
		...
		$strSql = "
			...
			WHERE
				R1.CHAT_ID = ".$toChatId." AND R1.USER_ID = ".$fromUserId."  ".$limitById." 
				#LIMIT#
	...
	public static function GetRelationByUserId($fromUserId, $toUserId)
	{
		global $DB;
	...
	public function Add($arParams)
	{
		...
		if ($chatId > 0)
		{
			...
			foreach ($arUserId as $userId)
			{
				...
				\Bitrix\IM\RelationTable::add(array(
					"CHAT_ID" => $chatId,
