...
BX.IM = function(domNode, params)
{
	...
	this.revision = 57; // api revision - check include.php
	...
	else if (type == 'contactList')
	{
		...
		if (this.recentList || userIsChat)
		{
			...
			menuItems = [
				...
				{icon: 'bx-messenger-menu-hide-'+(userIsChat? 'chat': 'dialog'), text: BX.message('IM_M_HIDE_'+(userIsChat? (this.chat[userId.toString().substr(4)].style == 'group'? 'CHAT': 'CALL'): 'DIALOG')), onclick: BX.delegate(function(){ BX.MessengerCommon.recentListHide(userId); this.closeMenuPopup();}, this)},
			...
			{text: BX.message("IM_F_DELETE"), onclick: BX.delegate(function(){
				...
				this.BXIM.openConfirm(deleteSelf? BX.message('IM_F_DELETE_SELF_CONFIRM'): BX.message('IM_F_DELETE_CONFIRM'), [
					new BX.PopupWindowButton({
						...
						className : "popup-window-button-accept",
						events : { click : BX.delegate(function() {
							this.disk.delete(chatId, fileId, {boxId: 'im-file-history-panel'});
...
BX.Messenger.prototype.editMessageAjax = function(id, text)
{
	...
	text = BX.MessengerCommon.prepareMention(this.currentTab, text);
...
BX.Messenger.prototype.showNotifyBlock = function(messageParams)
{
	...
	var isCall = isChat && this.chat[recipientId.substr(4)] && this.chat[recipientId.substr(4)].style == 'call';
					...
					BX.create('span', {
						props : {className : "bx-notifier-item-avatar"}, children : [
							BX.create('img', {
								...
								attrs : {src : isChat? this.chat[recipientId.substr(4)].avatar: this.users[senderId].avatar}
							})
						]
					}),
					BX.create("a", {
	...
	if (this.desktop.ready())
	{
		console.log(notifyHtmlNode.getAttribute("data-messageId"));
		var messsageJs =
			'var notify = BX.findChildByClassName(document.body, "bx-notifier-item");'+
			'notify.style.cursor = "pointer";'+
			'BX.bind(notify, "click", function(){BX.desktop.onCustomEvent("main", "bxImClickNewMessage", [notify.getAttribute("data-userId")]); BX.desktop.windowCommand("close")});'+
...
BX.IM.DiskManager.prototype.delete = function(chatId, fileId, params)
{
	if (!this.files[chatId] || !this.files[chatId][fileId])
		return false;
