...
BX.ImMobile.prototype.initPageAction = function (params)
{
	...
	BX.addCustomEvent("onOpenPush", BX.delegate(function (push)
	{
		...
		try
		{
			push.params = JSON.parse(push.params);
		}
		catch(e)
		{
			push.params = {'TAG': push.params};
		}

		if (push.params.TAG.substr(0, 8) == 'IM_MESS_')
		{
			var userId = parseInt(push.params.TAG.substr(8));
			if (userId > 0)
			{
		...
		else if (push.params.TAG.substr(0, 8) == 'IM_CHAT_')
		{
			var chatId = parseInt(push.params.TAG.substr(8));
			if (chatId > 0)
			{
				BXMobileApp.PageManager.loadPageUnique({
		...
		else if (push.params.TAG.substr(0, 6) == 'IMINV_')
		{
			var arg = push.params.TAG.split("_");
			...
			onsuccess: BX.delegate(function (data)
			{
				...
				if (data.ERROR.length == 0)
				{
					...
					if (data.COUNTERS && typeof(data.COUNTERS) == 'object')
					{
						...
						BXMobileApp.onCustomEvent('onImUpdateCounter', [data.COUNTERS, obZeroDate]);
					}
