;(function (window)
{
	...
	BX.MessengerCommon.prototype.contactListPrepare = function(params)
	{
		...
		if (viewChat)
		{
			...
			groupChat.sort(function(a, b) {
				i = BXIM.messenger.chat[a].name;
				ii = BXIM.messenger.chat[b].name;
	...
	BX.MessengerCommon.prototype.recentListPrepare = function(params)
	{
		...
		for (var i = 0; i < this.BXIM.messenger.recent.length; i++)
		{
			...
			{
				if (item.userIsChat)
					description = user.style == 'call'? BX.message('IM_CL_PHONE'): BX.message('IM_CL_CHAT');
				else
	...
	BX.MessengerCommon.prototype.recentListElementToTop = function(userId)
	{
		...
		if (!userFound)
		{
			...
			if (!messageText)
			{
				...
				if (userParam.style == 'chat')
				{
					messageText = BX.message('IM_CL_CHAT');
				}
				else if(userParam.style == 'call')
				{
	...
	BX.MessengerCommon.prototype.pullEvent = function()
	{
		BX.addCustomEvent((this.isMobile()? "onPull-im": "onPullEvent-im"), BX.delegate(function(command,params)
		{
			...
			else if (command == 'notify') // TODO mobile
			{
				...
				if (params.settingName == "main|rating_vote" && params.original_tag.substr(0,10) == "RATING|IM|")
				{
